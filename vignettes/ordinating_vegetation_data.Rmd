...
Remove reccords with missing abundance or missing plantsym
```{r}
CA792_veg_data |> dplyr::filter(!is.na(pct_cover) | !is.na(plantsym))
```
...
```{r, message=FALSE}
  ...
  bar <- ggplot2::ggplot(site.scrs, ggplot2::aes(x = NMDS1, y = NMDS2)) +
    ggplot2::geom_point(ggplot2::aes(color = factor(ecositeid),
                   text = usiteid)) +
    ggplot2::labs(title = "Presence absence NMDS", color = "Ecosite") +
    ...
    ggplot2::geom_segment(data = env.scores, ggplot2::aes(x = 0, xend = NMDS1, y = 0,
                                        yend = NMDS2),
                 arrow = ggplot2::arrow(length = ggplot2::unit(0.25, "cm")),
                 colour = "grey10", lwd = 0.3) +
    ggplot2::geom_text(data = env.scores, ggplot2::aes(x = NMDS1, y = NMDS2,
                                     label = env.variables), cex = 4)

  plotly::ggplotly(bar, tooltip = c("text", "color"), width = 900, height = 900)
