...
When analyzing ecological communities, it is often helpful to work with summarized soil properties that are not directly accessible from pedon data. Properties of interest could include depth, texture, color, rock fragments, pH, etc. Users may be interested in these properties within particular depth ranges, across the entire soil profile, or within a particular master horizon. An example could be weighted average clay percentage in the first 100 cms or thickness of the surface O horizons (i.e., using O master horizon). `summarize_pedon_soil_properties()` generates a variety of summarized properties that are useful for ecological analysis. I would like to develop this function further, so if you have other ideas of properties to summarize, please share! 

```{r, eval=FALSE}
# run summaries from NASIS 
CA792_pedon_summary <- ecositer::summarize_pedon_soil_properties(SS = TRUE,
                                          byDepth = list(c(0, 25), c(0, 50)))

# look at some of the new columns generated
CA792_pedon_summary[, setdiff(names(CA792_pedon_summary[["full_profile"]]), names(CA792_pedon_data))[c(15,19,45,60)]] |> head()
...
# perform pedon soil property summaries

data(CA792_pedon_data)

head(CA792_pedon_data)

# test <- ecositer::summarize_pedon_soil_properties(SS = FALSE,
