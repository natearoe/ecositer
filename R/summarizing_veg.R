...
summarize_veg <- function(veg_df,
  ...
  if(agg_abund){
    veg_df <- ecositer::QC_aggregate_abundance(veg_df = veg_df)
  }
  ...
  ecosite_species_sum <- veg_df |>
    dplyr::group_by(vegplotid, ecostateid, ecositeid, plantsciname) |> dplyr::summarise(sum(pct_cover, na.rm = TRUE)) |>
    dplyr::rename(total_plot_cover = `sum(pct_cover, na.rm = TRUE)`) |>
  ...
  ecosite_strata_sum <- veg_df |>
    dplyr::group_by(vegplotid, ecostateid, ecositeid, akstratumcoverclass) |> dplyr::summarise(sum(pct_cover, na.rm = TRUE)) |>
    dplyr::rename(plot_strata_cover = `sum(pct_cover, na.rm = TRUE)`) |>
  ...
  for(i in unique(veg_df$ecositeid[!is.na(veg_df$ecositeid)])){
    ...
    for(j in ecosite_list[[i]][["Raw_data"]]$ecostateid[!is.na(ecosite_list[[i]][["Raw_data"]]$ecostateid)] |> unique()){
      ...
      ecosite_strata_sum_i_state <- ecosite_strata_sum[ecosite_strata_sum$ecositeid == i &
                                                         ecosite_strata_sum$ecostateid == j, ]
      ...
      ecosite_species_sum_i_state <- ecosite_species_sum[ecosite_species_sum$ecositeid == i &
                                                           ecosite_species_sum$ecostateid == j, ]
